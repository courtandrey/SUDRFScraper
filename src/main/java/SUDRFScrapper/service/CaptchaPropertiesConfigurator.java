...
import static SUDRFScrapper.service.Constants.PATH_TO_CAPTCHA;
...
public class CaptchaPropertiesConfigurator {
    ...
    private Properties getProps() {
        ...
        try {
            if (Files.notExists(Path.of(String.format(PATH_TO_CAPTCHA,cc.getRegion())))) {
                Files.createFile(Path.of(String.format(PATH_TO_CAPTCHA,cc.getRegion())));
            }
            properties.load(new FileReader(String.format(PATH_TO_CAPTCHA,cc.getRegion())));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return properties;
    ...
    private void refresh() {
        try {
            props.store(new FileWriter(String.format(PATH_TO_CAPTCHA,cc.getRegion())),"");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
